...
solution_array = np.load('pde/advec_diff.npy')
solution_array = solution_array.reshape(513, 513).T[::-1, :]
Nx, Ny = solution_array.shape
def ex(x):
    x = np.atleast_2d(x)  # Ensure input is at least 2D
    x_clipped = np.clip(x, 0, 1)  # Ensure x is within [0,1]

    # Convert normalized coordinates [0,1] to nearest grid indices
    x_idx = np.round(x_clipped[:, 0] * (Nx - 1)).astype(int)
    y_idx = np.round(x_clipped[:, 1] * (Ny - 1)).astype(int)

    # Fetch the nearest value
    result = solution_array[x_idx, y_idx]

    # Return scalar if input was (2,)
...
class Kernel(GaussianKernel):
    ...
    def gauss(self, x, s, xhat):
        ...
        if self.mask:
            term1 = jnp.power(xhat[0] - self.D[0, 0], .2)  # (x_1 - 0)^a
            term2 = jnp.power(self.D[0, 1] - xhat[0], 1)  # (x_1 - 1)^b
            term3 = jnp.power(xhat[1] - self.D[1, 0], .2)  # (x_2 - 0)^c
            term4 = jnp.power(self.D[1, 1] - xhat[1], .2) # (x_2 - 1)^d
            mask = term1 * term2 * term3 * term4
            # mask = jnp.sqrt(mask)
            output = output * mask
        return output
    ...
    def P_gauss_X_c(self, X_shape, X, S, c, xhat):
        """
        Compute the PDE operator of the Gaussian kernel. Advection diffusion equation here.
        """
        return - self.diff * self.Lap_gauss_X_c(X, S, c, xhat) + jnp.dot(self.b, jax.grad(self.gauss_X_c, argnums=3)(X, S, c, xhat))
    
    @partial(jax.jit, static_argnums=(0,))
    def DP_gauss(self, x, s, xhat, u):
        return - self.diff * jnp.trace(jax.hessian(self.gauss, argnums=2)(x, s, xhat)) + jnp.dot(self.b, jax.grad(self.gauss, argnums=2)(x, s, xhat))
...
class ProblemNNLapVar:
    def __init__(self, alg_opt):
        ...
        self.test_int, self.test_bnd = self.sample_obs(200)
