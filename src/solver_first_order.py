...
from .utils import computeProx, Phi, compute_rhs, compute_y, compute_errors, merge_kernel_cluster
import jax 
...
def solve(p, y_ref, alg_opts):
    ...
    for k in range(1, max_step  + 1):
        ...
        coef = 0.01
        ck_inv = jnp.where(jnp.abs(ck) > 1e-14, 1 / jnp.abs(ck), 0)
        ck_long = coef * jnp.tile(jnp.abs(ck), dim)
        fisher_infodiag_compact = jnp.concatenate((ck_inv, ck_long))[compact_ind]
        II = (1 / alpha) * jnp.diag(fisher_infodiag_compact)
        ...
        while not has_descent and theta > 1e-8:
